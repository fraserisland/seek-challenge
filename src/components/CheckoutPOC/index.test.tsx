...
describe('CheckoutPOC component', () => {
  it('renders elements correctly', () => {
    ...
    expect(screen.getByRole('heading', { name: /current customer: default/i })).toBeInTheDocument()

    expect(screen.getByRole('button', { name: /default/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /axil coffee roasters/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /second bite/i })).toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: /total \$0/i })).toBeInTheDocument()
  })

  it('updates the current customer type', () => {
    render(<CheckoutPOC />)

    const customerType = screen.getByRole('heading', { name: /current customer: default/i })
  ...
  it('displays the added checkout products total', () => {
    ...
    const premiumAddBtn = within(screen.getByText(/premium/i)).getByRole('button', { name: /add/i });
    const stadoutAddBtn = within(screen.getByText(/standout/i)).getByRole('button', { name: /add/i });
