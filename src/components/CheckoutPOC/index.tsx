  ...
  return (
    <main>
      <section>
        <h2>Current customer: {currentCustomer}</h2>
        {
          Object.keys(Customers).map((c) => {
            ...
            return <button disabled={currentCustomer === customer} name={customer} key={customer} onClick={handleCustomerUpdate}>{customer}</button>
          })
        }
      </section>
      ...
      <section>
        <h2>Products:</h2>
        {
          (Object.keys(products) as Array<keyof typeof Product>).map(p => (
            <div key={p}>
              {products[p].name}
              <button name={p} onClick={handleAdd}>add</button>
              <hr />
            </div>
          ))
        }
      ...
      <section>
        ...
        {
          items.map(item => <p key={`item-${item.name}`}>{item.name}, {item.qty}, {item.price}</p>)
        }
      </section>
    </main>
  )
